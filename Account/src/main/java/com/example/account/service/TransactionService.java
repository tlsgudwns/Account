...
public class TransactionService {
    ...
    //transactionRepository 에는 transaction 엔티티가 저장되어야한다
    //                          (transaction 도메인이 저장되어야한다)
    private final AccountUserRepository accountUserRepository;
    private final AccountRepository accountRepository;

    @Transactional //이 어노테이션을 붙여놓으면 중괄호 내에있는것들이 모두 한번에 작동된다
                //작동하면 전부 동작하고 안하면 전부 동작하지않는것
    public TransactionDto useBalance(Long userId, String accountNumber,
                                     Long amount) {
        AccountUser user = accountUserRepository.findById(userId)
                .orElseThrow(() -> new AccountException(ErrorCode.USER_NOT_FOUND));
        //userId와 accountUserRepository를 통해서 user(도메인)찾아내기
