...
class TransactionServiceTest {
    ...
    void successUseBalance() {
        ...
        //실제로 save가 되는지 체크하기위해서 ArgumentCaptor를 사용한다

        //when
        TransactionDto transactionDto = transactionService.useBalance(1L,
                "1000000000", USE_AMOUNT);
        //when에서의 전달인자의 accountNumber또는 userId칸에 뭐가 들어가든 상관없는
        // 이유는 이 transactionService이
        //useBalance를 호춣할때 그 속의 accountUserRepository, accountRepository
        //에서 찾아와지는값은 뭘 넣든 우리가 given에서 anyLong, anyString들에 어떤걸
        //집어넣든 willReturn한 값을 받아와지도록 설정했기때문이다
        //amount도 뭐가 들어가든 상관없는듯
        //transactionService.useBalance의 리턴부분이
        //return TransactionDto.fromEntity(saveAndGetTransaction(S, account, amount));
        //이고 입력을 뭘하든 이 메소드의 리턴값은 위에서 given으로 설정한값이 될것이다

        //then
        verify(transactionRepository, times(1)).save(captor.capture());
        //실제로 save가 되는지 체크하기위해서 ArgumentCaptor, verify를 사용한다
    ...
    void successCancelBalance() {
        ...
        Transaction transaction = Transaction.builder()
                .account(account)
                .transactionType(USE)   //원래거래를 찾기위한 transaction객체니까
                .transactionResultType(S) //USE가 들어가는게 맞다
                .transactionId("transactionId")
                .transactedAt(LocalDateTime.now())
        ...
        given(transactionRepository.save(any()))
                .willReturn(Transaction.builder()
                        .account(account)
                        .transactionType(CANCEL) //취소된 거래내역의 저장을 설정하는 GIVEN
                        .transactionResultType(S)//이니까 CANCEL이 들어가는게 맞다
